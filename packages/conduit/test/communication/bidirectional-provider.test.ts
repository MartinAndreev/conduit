...
test("ACP stdio negotiates, configures a model, streams chunks, and returns the prompt result", async () => {
  ...
  const submittedPromptFile = path.join(directory, "submitted-prompt.txt");
  const responseCapturePathFile = path.join(
    directory,
    "response-capture-path.txt",
  );
  await writeFile(
    executable,
    `#!/usr/bin/env node
...
const finishPrompt = () => {
  require("node:fs").writeFileSync(responseCapture, ${JSON.stringify(finalResponse)});
...
const handle = (input) => {
  ...
  if (input.method === "session/new") { responseCapture = input.params.mcpServers[0].env.find((item) => item.name === "CONDUIT_AGENT_RESPONSE_CAPTURE").value; const ready = input.params.mcpServers[0].env.find((item) => item.name === "CONDUIT_AGENT_RESPONSE_READY").value; require("node:fs").writeFileSync(ready, "ready\\n"); require("node:fs").writeFileSync(${JSON.stringify(responseCapturePathFile)}, responseCapture); return send({ jsonrpc: "2.0", id: input.id, result: { sessionId: "SESSION_REDACTED", configOptions: [{ id: "model", options: [{ value: "cheap/model" }] }, { id: "effort" }] } }); }
  ...
  if (input.method === "session/prompt") {
    promptId = input.id;
    require("node:fs").writeFileSync(${JSON.stringify(submittedPromptFile)}, input.params.prompt[0].text);
...
`,
  ...
  try {
    ...
    const submittedPrompt = await readFile(submittedPromptFile, "utf8");
    assert.ok(submittedPrompt.includes(JSON.stringify(input)));
    assert.match(submittedPrompt, /conduit_submit_agent_response tool/);
    assert.ok(submittedPrompt.endsWith(agentResponseContractPrompt("tool")));
    assert.ok(events.some((event) => event.type === "usage"));
    assert.ok(
      events.some(
        (event) =>
          event.type === "protocol-lifecycle" &&
          event.payload.state === "response-tool-ready",
      ),
    );
    assert.ok(
      events.some((event) => event.type === "final-response-candidate"),
    );
    assert.equal(
      events.filter((event) => event.type === "final-response-candidate")
        .length,
      1,
    );
    assert.ok(
    ...
    const responseCapturePath = await readFile(responseCapturePathFile, "utf8");
    await assert.rejects(readFile(responseCapturePath));
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});

test("ACP requests one same-session completion retry when the model omits the response tool", async () => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "conduit-acp-followup-"),
  );
  const executable = path.join(directory, "fake-acp-followup");
  await writeFile(
    executable,
    `#!/usr/bin/env node
let buffer = "";
...
const handle = (input) => {
  ...
  if (input.method === "session/new") {
    responseCapture = input.params.mcpServers[0].env.find((item) => item.name === "CONDUIT_AGENT_RESPONSE_CAPTURE").value;
    const ready = input.params.mcpServers[0].env.find((item) => item.name === "CONDUIT_AGENT_RESPONSE_READY").value;
    require("node:fs").writeFileSync(ready, "ready\\n");
    return send({ jsonrpc: "2.0", id: input.id, result: { sessionId: "SESSION_REDACTED", configOptions: [] } });
  }
  if (input.method === "session/prompt") {
    promptCount += 1;
    if (promptCount === 2) require("node:fs").writeFileSync(responseCapture, ${JSON.stringify(finalResponse)});
    return send({ jsonrpc: "2.0", id: input.id, result: { stopReason: "end_turn" } });
  }
};
process.stdin.on("data", (chunk) => {
  buffer += chunk;
  ...
  try {
    ...
    assert.equal(
      events.filter(
        (event) =>
          event.type === "warning" &&
          event.payload.code === "AGENT_RESPONSE_TOOL_RETRY",
      ).length,
      1,
    );
    assert.equal(
      events.some(
        (event) =>
          event.type === "warning" &&
          event.payload.code === "AGENT_RESPONSE_TOOL_MISSING",
      ),
      false,
    );
  } finally {
