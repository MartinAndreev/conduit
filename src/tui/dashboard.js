...
export function splitPatchFiles(patch) {
  const files = patch
    .split(/(?=^diff --git )/m)
    .filter(Boolean)
    .map((diff) => ({
...
export function summarizeTranscript(log) {
...
async function readRoleLog(projectRoot, config, run, role) {
  return readFile(
