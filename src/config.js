...
export const CONFIG_FILE = "conduit.yml";
...
// The generated file is intentionally narrow YAML so it stays dependency-free.
export function serializeConfig(config = defaultConfig) {
  const lines = [
    "version: 1",
    "specsDir: specs",
    "stateDir: .conduit",
...
export function parseConfig(yaml) {
  const config = { ...defaultConfig, roles: {} };
  let roleName;
  let inSkill = false;
  for (const rawLine of yaml.split(/\r?\n/)) {
    ...
    if (indent === 0) {
      if (key !== "roles") config[key] = scalar(rawValue ?? "");
      roleName = undefined;
      inSkill = false;
    } else if (indent === 2) {
      roleName = key;
      config.roles[roleName] = { skill: {} };
      inSkill = false;
    } else if (indent === 4 && roleName) {
      if (key === "skill") inSkill = true;
      else {
        config.roles[roleName][key] = scalar(rawValue ?? "");
        inSkill = false;
      }
    } else if (indent === 6 && roleName && inSkill) {
      config.roles[roleName].skill[key] = scalar(rawValue ?? "");
    }
  }
  return config;
}
